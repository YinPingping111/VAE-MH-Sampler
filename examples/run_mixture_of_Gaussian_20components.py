...
VAE_MH_sampler = algorithm.MH_VAE(log_likelihood=log_likelihood, dim=dim, input_dim=input_dim, inter_dim=inter_dim,
                         inter_dim1=inter_dim1, latent_dim=latent_dim, batch_size=batch_size, epochs=epochs,
                         low_bound=low_bound, high_bound=high_bound,
                         initial_samples=initial_samples, retrains=retrains, samples_per_retrain=samples_per_retrain,
                         outdir=outdir, sigma=sigma, vae_prob=vae_prob, bins=bins, plot_initial=plot_initial, lr=lr)
